...
from config import from_dir,result_dir,outline_dir,FileDirMaker,road_version #常量
...
if __name__ == "__main__":
    ...
    city_ls = os.listdir(from_dir)#城市选择导的顺序——按字母表顺序排序
    print(city_ls)
    ...
    for city in city_ls:
        ...
        dirs = FileDirMaker(city,old_vno,new_vno,from_dir,result_dir,outline_dir)
        #备选路径——备选版本号1
        dirs_altermate1 = FileDirMaker(city,old_vno_altermate1,new_vno,from_dir,result_dir,outline_dir)
        #备选路径——备选版本号2
        dirs_altermate2 = FileDirMaker(city,old_vno_altermate2,new_vno,from_dir,result_dir,outline_dir)
        
        #新建成果中的城市文件夹
        output_city_file = os.path.join(result_dir, city_c) 
        MakeDir(output_city_file)

        output_file = os.path.join(result_dir, city_c, road_version) #新建成果中的【道路N次】文件夹
        MakeDir(output_file)
